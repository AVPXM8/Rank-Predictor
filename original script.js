...
function hasUnlimitedAccess(phoneNumber) {
    ...
    console.log(`📋 Target admin phone: 9354368207`);
    console.log(`🔍 Phone match check: "${phoneNumber}" === "9354368207" = ${phoneNumber === '9354368207'}`);
    
    const isAdmin = PREDICTION_CONFIG.ADMIN_UNLIMITED && (phoneNumber === '9354368207');
...
const sendSMS_MSG91_HTTP = async (phoneNumber, otp) => {
    try {
        ...
        const message = `Your Maarula Classes Rank Predictor OTP is ${otp}. Valid for 10 minutes. Please do not share with anyone.`;
...
// new Route do not require signup procedure
// ADD THIS NEW ROUTE TO YOUR SERVER.JS FILE

// New route to register a user without OTP verification
app.post('/api/register-guest', async (req, res) => {
    try {
        ...
        if (user) {
            // User exists, update their name if it was pending and log them in
            user.fullName = fullName.trim();
            console.log(`User ${phoneNumber} already exists. Logging in.`);
        } else {
            // Create a new user
            user = new User({
                phoneNumber,
                fullName: fullName.trim(),
                isVerified: false, // You can use this field to distinguish between OTP-verified and guest users
            });
            console.log(`New user ${phoneNumber} created.`);
        }
        
        // Create a session token for them immediately
        ...
        console.error('Register Guest Error:', error.message);
...
// // 🎯 PREDICTION LIMIT CONFIGURATION
// const PREDICTION_CONFIG = {
//     LIMIT_PER_PHONE: 50, // Change to 2 for 2 predictions limit
...
//            (phoneNumber === '9935985550' || // Your number
...
//         // Send SMS
//         const smsMessage = `Your Maarula Classes Rank Predictor OTP is: ${otp}. Valid for 10 minutes. Do not share with anyone.`;
//         const smsResult = await sendSMS(phoneNumber, smsMessage);
